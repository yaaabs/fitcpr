<style>
  #login-nav {
    background-color: #00522A; /* FEU Tech green */
...
#top-Nav {
    background-color: #fff; /* FEU Tech green */
    
}

#top-Nav .nav-link {
    color: #000; /* White text for navbar links */
    font-weight: bold;
}


/* #top-Nav .nav-link.active {
    border-bottom: 2px solid #FDB813; /* FEU Tech yellow highlight for active links */
    color: #FDB813; /* Ayaw maalis ng fcking active e */
} */
...
<!-- Navbar -->
      ...
      <nav class="bg-00522A w-100 px-2 py-1 position-fixed top-0" id="login-nav">
        <div class="d-flex justify-content-between w-100">
          <div>
          <span class="mr-2 text-black">
          <a href="mailto:<?= $_settings->info('email') ?>">
    <i class="fa fa-envelope mr-1"></i> <?= $_settings->info('email') ?>
</a>
    ...
    <?php if($_settings->userdata('id') > 0): ?>
        ...
        <span class="mx-2">
            Hello, <?= !empty($_settings->userdata('firstname')) ? $_settings->userdata('firstname') : $_settings->userdata('email') ?>!
        </span>
        <span class="mx-1">
            <a href="<?= base_url.'classes/Login.php?f=student_logout' ?>"><i class="fa fa-power-off"></i></a>
        </span>
    <?php else: ?>
    ...
    <ul aria-labelledby="dropdownSubMenu1" class="dropdown-menu border-0 shadow" style="left: 0px; right: inherit;">
        ...
        ?>
        <li class="dropdown-header"><?= ucwords($department['name']) ?></li>
        <?php 
            // Fetch curriculums under each department
            $curriculums = $conn->query("SELECT * FROM curriculum_list WHERE status = 1 AND department_id = '{$department['id']}' ORDER BY name ASC");
            while ($curriculum = $curriculums->fetch_assoc()):
        ?>
        <li>
            <a href="./?page=projects_per_curriculum&id=<?= $curriculum['id'] ?>" class="dropdown-item"><?= ucwords($curriculum['name']) ?></a>
        </li>
        <?php endwhile; ?>
        <li class="dropdown-divider"></li>
        <?php endwhile; ?>
