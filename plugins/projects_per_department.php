    ...
    <div class="col-12">
        <div class="card card-outline card-warning shadow rounded-0">
            <div class="card-body rounded-0">
                <h2 class="text-center mb-4">Archive List of <?= isset($department['name']) ? $department['name'] : "" ?> </h2>
                <p class="text-center"><small><?= isset($department['description']) ? $department['description'] : "" ?></small></p>
                <hr class="bg-navy">
                <?php 
                ...
                $wheredid = " and department_id = '{$id}' ";
                $students = $conn->query("SELECT * FROM `student_list` where id in (SELECT student_id FROM archive_list where `status` = 1 and curriculum_id in (SELECT id from curriculum_list where department_id = '{$id}'))");
                $student_arr = array_column($students->fetch_all(MYSQLI_ASSOC),'email','id');
                $count_all = $conn->query("SELECT * FROM archive_list where `status` = 1 and curriculum_id in (SELECT id from curriculum_list where department_id = '{$id}')")->num_rows;    
                $pages = ceil($count_all/$limit);
                $archives = $conn->query("SELECT * FROM archive_list where `status` = 1 and curriculum_id in (SELECT id from curriculum_list where department_id = '{$id}') order by unix_timestamp(date_created) desc {$paginate}");    
                ?>
